...
class _FolderFilesScreenState extends State<FolderFilesScreen> {
  ...
  Future<void> fetchFiles() async {
    ...
    final userKey = userProvider.user?.user_id;

    if (userKey != null) {
      final response = await http.get(Uri.parse(
        '${API.baseUrl}/api/${widget.folderType}-files/${widget.folderId}?user_id=$userKey',
      ));
    ...
    if (userKey != null) {
      ...
      try {
        final response = await http.put(url,
            body: jsonEncode({'file_name': newName, 'user_id': userKey}),
    ...
    if (userKey != null) {
      ...
      try {
        final response = await http.delete(url,
            body: jsonEncode({'user_id': userKey}),
            headers: {'Content-Type': 'application/json'});
        if (response.statusCode != 200) {
