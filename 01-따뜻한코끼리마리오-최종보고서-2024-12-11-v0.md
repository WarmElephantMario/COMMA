...
<h3 id="(1) 평가항목선정"> 1. 평가 항목 선정</h3>

| 항목 | 평가 항목 | 평가 항목 선정 이유 |
| --- | --- | --- |
| 주 기능 1 -<br> 대체텍스트 생성 기능 | • 강의 자료 내 텍스트 인식 및 추출의 정확도 (OCR)<br>• 강의 자료 내 시각적 자료(표, 그래프, 사진)에 대한 대체텍스트의 설명 적절성 | 시각장애인 학습자는 스크린 리더를 통해 강의 자료의 텍스트 정보를 접근하기 때문에, 강의 자료 속의 글자를 정확히 인식하는 것이 필수적이다.<br>또한, 시각 자료의 의미를 적절히 텍스트로 전달해야 학습자가 자료의 핵심 내용을 온전히 이해할 수 있다. |
| 주 기능 2 -<br> 실시간 자막 생성 기능 | • 실시간 자막의 출력 속도<br>• 생성되는 실시간 자막의 정확도 | 자막이 실시간으로 출력되지 않으면 청각장애 학습자가 강의를 따라가기 어렵기 때문에, 출력 속도가 중요한 평가 항목이다.<br>또한, 부정확한 자막은 강의 내용에 대한 오해를 초래할 수 있기 때문에 자막의 정확성 역시 평가 항목으로 선정하였다. |
| 주 기능 3 -<br> 콜론 생성 기능 | • 속기록과 강의 자료 사이 문단 매칭의 정확성<br>• 소요되는 강의 자료의 학습 시간 | 속기 문장을 잘못된 슬라이드에 매칭할 경우 학습자가 강의 내용을 복습할 때 혼란이 가중되므로, 매칭의 정확성은 학습 효율 증진에 중요한 요소이다.<br>또한, 학습 자료 생성 과정에서 소요 시간이 너무 길면 학습 비효율이 증가하므로 소요 시간도 평가 항목으로 선정하였다. |

<h3 id="(1) 대체텍스트생성기능평가"> 1. 대체텍스트 생성 기능 평가</h3>
...
| (3) 평가 방식   | 1번 항목 ) 각 강의 자료 당 총 3번 대체텍스트 생성 후, 오류 단어의 개수를 평가기준을 통해 점수를 측정한다.<br>
    1. 네 가지 강의 자료를 대상으로 평가를 진행한다. <br>
    컴퓨터 네트워크01 (20페이지) <br>
    여성과 예술01 (15페이지)<br>
    기초 통계학01 (20페이지)<br>
    시스템소프트웨어실습01 (20페이지)<br>
    2. 각 강의자료의 대체 텍스트로 생성한 후, 실제 강의 자료의 단어와 비교한다.<br>
    3. 오류 단어(잘못된 인식, 오탈자, 누락 등)의 개수를 확인한다.<br>
    4. 슬라이드별 오류 단어 개수를 평가기준으로 점수를 매긴다.<br><br>
2번 항목 ) 총 6명의 시각장애 학우를 대상으로 베타테스트 진행 및 대면 인터뷰를 진행했다.<br>
    1. 시각장애 학우가 수강 중인 강의자료(PDF)를 활용하여 대체 텍스트를 생성한다.<br>
    2. 이미지 자료(그림, 그래프, 표)에 대해 생성한 대체텍스트 내용을 평가기준에 따라 점수를 매긴다. |
...
<h3 id="(2) 실시간자막생성기능평가"> 2. 실시간 자막 생성 기능 평가</h3>
...
| (3) 평가 방식 | 1번 항목 ) 1번 항목 ) 총 6명의 청각장애 및 비장애 도우미 학우를 대상으로 베타테스트 진행 후 비대면 설문 조사를 진행한다. <br>
    1. 청각장애 학우와 비장애 도우미 학우에게 강의자료를 업로드하여 실시간 자막을 생성하는 방법을 안내한다.<br>
    2. 학우들이 일주일간 수강 중인 수업에 실시간 자막 기능을 사용하도록 한다.<br>
    3. 일주일동안 사용 후, 평가기준에 따라 비대면 설문조사를 통해 자막의 정확성과 만족도를 조사한다.<br><br> 
    2번 항목 ) 강의자료 당 총 3번 실시간 자막 생성 한 후 CER 측정한다.<br>
    1.네 가지 강의 자료를 대상으로 평가를 진행한다. <br>
    컴퓨터 네트워크01 (20페이지) <br>
    여성과 예술01 (15페이지)<br>
    기초 통계학01 (20페이지)<br>
    시스템소프트웨어실습01 (20페이지)<br>
    2.생성된 자막과 정답 자막 대본을 비교하여 CER(Character Error Rate)을 측정한다.<br>
    3. CER 측정은 전용 함수 프로그램을 사용해 수행하며, 평가기준에 따라 점수를 매긴다. |
| (4) 평가 결과 | **1번 항목 ) 본 어플의 실시간 자막 기능 사용 시, 자막 생성 속도가 실시간 성으로 적절히 출력되는지 평가**<br>- 평균 점수 : 9.17점<br>- 결론 : 평균 점수가 9.17점으로 높은 평가를 받은 것을 고려할 때, 본 어플의 실시간 자막 기능은 대부분의 사용자들이 실시간성 측면에서 적절하다고 평가함. 일부 지연되는 문장이 있는 경우는 강의 환경, 마이크 품질 또는 주변 소음과 같은 외부 요인에 의해 발생할 수 있음. 그럼에도 불구하고, 전반적으로 실시간 자막 생성은 원활하게 이루어짐.<br><br><img src="readme/자막평가결과.png" width="700" /><br><br>**2번 항목 ) 생성된 실시간 자막의 정확도 평가**<br>- CER 측정 결과 : 10.870% (우수)<br>- 결론 : 측정된 CER 값은 10.870%로, 평가 기준에 따라 우수에 해당함. 이는 생성된 실시간 자막이 정확하며, 대부분의 발화를 효과적으로 반영하고 있음을 나타냄.<br><br> |

<h3 id="(3) 콜론생성기능평가"> 3. 콜론 생성 기능 평가</h3>
...
| (3) 평가 방식 | 1번, 2번 항목 ) 총 12명의 시각장애, 청각장애 및 비장애 도우미 학우를 대상으로 베타테스트 진행 후 비대면 설문 조사를 진행한다.<br>
    1. 시각장애, 청각장애 학우와 비장애 도우미 학우에게 강의자료를 업로드하여 콜론을 생성하는 방법을 안내한다.<br>
    2. 학우들이 일주일간 수강 중인 수업에 콜론 기능을 사용하도록 한다.<br>
    3. 일주일동안 사용 후, 평가기준에 따라 비대면 설문조사를 통해 자막의 정확성과 만족도를 조사한다.   |
| (4) 평가 결과 | **1번 항목 ) 본 어플에서 생성한 콜론 파일이 강의 자료의 슬라이드에 해당하는 설명 문장을 적절히 구분하여 매칭하였는지 평가**<br>- 평균 점수 : 8.2점<br>- 결론 : 매칭의 정확성은 대체로 우수했으나, 대부분의 슬라이드에서 1-2문장 정도의 부정확한 매칭이 발생함. 이는 Overfitting 방지를 위해 한 텍스트 파일을 200토큰 단위 (약 2문장)로 묶는 방식에서 기인한 것으로, 전체적으로 정확도를 높이기 위한 최소 수준의 오류율임.<br><br><img src="readme/콜론구분결과.png" width="700" /><br><br> **2번 항목 ) 콜론 자료를 생성하는 데 소요되는 시간의 적절함 평가**<br>- 평균 점수 : 8.3점<br>- 결론 : 대부분의 경우 콜론 파일의 생성 시간이 효율적이고 짧은 수준으로 평가되었음. 사용자 경험에 의하면, 이는 학습 경험을 저해하지 않을 정도의 유의미하게 짧은 수준으로 평가됨.<br><br><img src="readme/콜론생성시간결과.png" width="700" /> <br><br> |
